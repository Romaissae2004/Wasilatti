...
const PharmacyList = ({ toggleCart }) => {
  const pharmacies = [
    { name: 'Doliprane 1000mg', prix: 35, img: '💊', pharma: 'PharmaDirect' },
    { name: 'Masque chirurgical x50', prix: 25, img: '😷', pharma: 'PharmaDirect' },
    { name: 'Vitamine C 500mg', prix: 60, img: '🍊', pharma: 'ParaPharma' },
    { name: 'Thermomètre digital', prix: 120, img: '🌡️', pharma: 'PharmaDirect' },
  ];

  return (
    <div className="mb-12">
      <h2 className="text-2xl font-display font-bold mb-6 flex items-center gap-2 dark:text-white">
        <span className="w-8 h-8 rounded-lg bg-green-100 dark:bg-green-900/30 flex items-center justify-center text-green-600 dark:text-green-400 text-sm">
          💊
        </span>
        Pharmacies
      </h2>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        {pharmacies.map((p, i) => (
          <div key={i} className="product-card bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 overflow-hidden shadow-sm hover:shadow-lg transition-all">
            <div className="relative h-40 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 flex items-center justify-center group">
